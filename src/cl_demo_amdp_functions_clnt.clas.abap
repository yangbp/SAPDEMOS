CLASS cl_demo_amdp_functions_clnt DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS get_scarr_spfli_for_cds
                  FOR TABLE FUNCTION demo_cds_get_scarr_spfli_clnt.
...
CLASS CL_DEMO_AMDP_FUNCTIONS_CLNT IMPLEMENTATION.
  ...
  METHOD select_get_scarr_spfli
         ...
         LANGUAGE SQLSCRIPT
         OPTIONS READ-ONLY
         USING cl_demo_amdp_functions_clnt=>get_scarr_spfli.
    SCARR_SPFLI_TAB =
    SELECT *
           FROM "CL_DEMO_AMDP_FUNCTIONS_CLNT=>GET_SCARR_SPFLI"(
                  clnt => :clnt,
                  carrid => :carrid );
