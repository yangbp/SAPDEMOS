CLASS cl_demo_amdp_functions DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS get_scarr_spfli_for_cds
                  FOR TABLE FUNCTION demo_cds_get_scarr_spfli.
...
CLASS CL_DEMO_AMDP_FUNCTIONS IMPLEMENTATION.
  ...
  METHOD select_get_scarr_spfli
         ...
         LANGUAGE SQLSCRIPT
         OPTIONS READ-ONLY
         USING cl_demo_amdp_functions=>get_scarr_spfli.
    SCARR_SPFLI_TAB =
    SELECT *
           FROM "CL_DEMO_AMDP_FUNCTIONS=>GET_SCARR_SPFLI"(
                  clnt => :clnt,
                  carrid => :carrid );
